...
function scoreDXY(changePct: number): SignalScore { return changePct < -0.3 ? 1 : changePct <= 0.3 ? 0 : -1; }
...
function applySnapshotSignals(signals: MacroSignal[], snapshotSignals: Record<string, number>, snapshotData: MacroAPIResponse): MacroSignal[] {
  // First apply live data formatting, then override scores from snapshot
